...
def main():
    ...
    densenet_model, ema_densenet_model = build_model()
    print(f"Build `{config.model_arch_name}` model successfully.")

    pixel_criterion = define_loss()
    print("Define all loss functions successfully.")

    optimizer = define_optimizer(densenet_model)
    ...
    if config.pretrained_model_weights_path:
        densenet_model, ema_densenet_model, start_epoch, best_acc1, optimizer, scheduler = load_state_dict(
            densenet_model,
    ...
    for epoch in range(start_epoch, config.epochs):
        train(densenet_model, ema_densenet_model, train_prefetcher, pixel_criterion, optimizer, epoch, scaler, writer)
        acc1 = validate(ema_densenet_model, valid_prefetcher, epoch, writer, "Valid")
        print("\n")
        ...
        save_checkpoint({"epoch": epoch + 1,
                         ...
                         "ema_state_dict": ema_densenet_model.state_dict(),
...
def build_model() -> [nn.Module, nn.Module]:
    ...
    ema_densenet_model = AveragedModel(densenet_model, avg_fn=ema_avg)

    return densenet_model, ema_densenet_model
