...
def build_model(model_arch_name: str, model_num_classes: int, device: torch.device) -> [nn.Module, nn.Module]:
    densenet_model = model.__dict__[model_arch_name](num_classes=model_num_classes)
    densenet_model = densenet_model.to(device=device, memory_format=torch.channels_last)
...
def main():
    ...
    densenet_model = build_model(args.model_arch_name, args.model_num_classes, device)
    print(f"Build `{args.model_arch_name}` model successfully.")

    # Load model weights
    densenet_model, _, _, _, _, _ = load_state_dict(densenet_model, args.model_weights_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_arch_name", type=str, default="densenet121")
    ...
    parser.add_argument("--model_weights_path", type=str, default="./results/pretrained_models/DenseNet121-ImageNet_1K-30a6e303.pth.tar")
